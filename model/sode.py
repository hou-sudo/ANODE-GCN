...
from model.modules import SC_GC, TemporalEncoder, GCN
...
class DiffeqSolver(nn.Module):
    def __init__(self, ode_func, method,num_point,asc_dim_trans, odeint_rtol=1e-4, odeint_atol=1e-5):
        ...
        self.linear_layer = nn.Linear(num_point+asc_dim_trans, num_point)
    ...
    def forward(self, first_point, time_steps_to_predict):
        ...
        pred_y = self.linear_layer(pred_y)
...
class SODE(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, ode_method='rk4',
                 ...
                 T=64, n_step=1, dilation=1, SAGC_proj=True, num_cls=10, asc_dim_trans = 1 ,
                 n_sample=1, backbone='transformer', tol=1e-3, adjoint=False):
        ...
        with torch.no_grad():
            ...
            self.n_step = n_step
            self.asc_dim_trans = asc_dim_trans
        ...
        a = torch.from_numpy(self.Graph.A_norm).cuda()
        aug_1 = torch.zeros(a.shape[0], asc_dim_trans).to("cuda:0")
        aug_0 = torch.zeros(asc_dim_trans, a.shape[1]+asc_dim_trans).to("cuda:0")
        a = torch.cat([a, aug_1], 1)
        a = torch.cat([a, aug_0], 0)

        ode_func = ODEFunc(base_channel, a, N=n_step, T=T).to(device)
        num_point
        asc_dim_trans
        self.diffeq_solver = DiffeqSolver(ode_func, method=method, num_point=num_point, asc_dim_trans =asc_dim_trans) if method != "RNN" else \
    ...
    def extrapolate(self, z_0, t, asc_dim_trans):
        '''
        z : n c t v
        '''
        B, C, T, V = z_0.size()
        ...
        aug = torch.zeros(z_0.shape[0], z_0.shape[1],asc_dim_trans).to("cuda:0")
       # Shape (batch_size, data_dim + augment_dim)
        x_aug = torch.cat([z_0, aug], 2)
        zs = self.diffeq_solver(x_aug, t) # z_i = 2, (b t), c, v 
    ...
    def forward(self, x):
        ...
        #x = self.to_joint_embedding(x_aug)
        ...
        # extrapolation 
        z_0, z_hat, z_hat_shifted = self.extrapolate(z, self.arange_n_step.to(z.dtype), self.asc_dim_trans)
        ...
        if self.n_step:
            ...
            a = torch.cat([z_0, z_hat_cls], dim=1)
