...
from loss import LabelSmoothingCrossEntropy, masked_recon_loss, smooth_l1_loss
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for x, y, mask, index in tbar:
            ...
            if self.arg.lambda_2:
                ...
                recon_loss = self.arg.lambda_2 * self.recon_loss2(x_hat, x_gt, mask_recon)
            ...
            if self.arg.lambda_3:
                ...
                feature_loss = self.arg.lambda_2 * self.recon_loss2(z_hat, z_0, mask_feature)
        ...
        AUC = np.mean([self.log_acc[i].avg.cpu().numpy() for i in range(10)])
        f.write(f"{epoch}-{AUC:.3f}-{self.log_cls_loss.avg:.3f}-{self.log_feature_loss.avg:.3f}-{self.log_recon_loss.avg:.5f}"+'\n')#删
        train_dict = {
        ...
        if save_model:
            ...
            torch.save(weights, '{}/runs-{}.pt'.format(Path(self.arg.work_dir),epoch+1))
    ...
    def eval(self, epoch, save_score=False, loader_name=['test']):
        ...
        for ln in loader_name:
                ...
                with torch.no_grad():
                    ...
                    for i in range(N_rec):
                        if N_rec == self.arg.n_step:
                            ...
                            mask_recon[i,:,:,:i,:,:] = 0.
                    mask_recon = rearrange(mask_recon, 'n b c t v m -> (n b) c t v m')
                    recon_loss = self.recon_loss2(x_hat, x_gt, mask_recon)
                    ...
                    z_hat = z_hat.view(N_step, B_, C, T, V)
                    mask_feature = (z_hat != 0.)
                    feature_loss = self.recon_loss2(z_hat, z_0, mask_feature)
            ...
            AUC = np.mean([self.log_acc[i].avg.cpu().numpy() for i in range(10)])
            f2.write(f"{epoch}-{AUC:.3f}-{self.log_cls_loss.avg:.3f}-{self.log_feature_loss.avg:.3f}-{self.log_recon_loss.avg:.5f}"+'\n')#删
            eval_dict = {
